  ...
  exercises
    ...
    .forEach((ex) => {
      ...
      nextUnlocks.push({
        ...
        val: `${ex.sessionsAtLevel}/${req}`,
        ok: false,
      })
    })

  MOCK_ARCS.filter((a) => a.status === 'active').forEach((arc) => {
  ...
  return (
    <div className="min-h-screen bg-s0">
      ...
      <PageHeader
        ...
        right={
          <div className="flex items-center gap-1.5">
            ...
            <span className="text-[11px] font-medium text-state-green">
              +{user.globalScoreDelta}
            </span>
          </div>
        }
      />
      ...
      <div className="mx-5 mb-4 bg-s1 border border-s3 rounded-2xl px-4 py-3">
        <p className="text-[9px] text-t4 uppercase tracking-widest mb-2.5">
          ...
          <div className="space-y-2">
            {topUnlocks.map((u, i) => (
              <div key={i} className="flex items-center gap-3">
                <span className={u.ok ? 'text-state-green' : 'text-t4'}>
                  {u.ok ? '✓' : '○'}
                </span>
      ...
      <div className="mx-5 mb-3">
        <BookCard
          ...
          onOpenQcm={
            progress.isComplete && !bookQcmPassed
      ...
      <div className="mx-5 mb-4 bg-s1 border border-s3 rounded-2xl overflow-hidden">
        <div className="px-4 py-3 border-b border-s2">
          <p className="text-[9px] text-t4 uppercase tracking-widest">
            Ordre de lecture · 0/{MOCK_BOOKS.length} terminés
          </p>
        </div>
        ...
        <div className="divide-y divide-s2">
          {MOCK_BOOKS.map((b, i) => {
            ...
            return (
              <div
                key={b.id}
                className={`flex items-center gap-3 px-4 py-3 ${!isUnlocked ? 'opacity-30' : ''}`}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <p
                    className={`text-xs font-medium ${
                      isCurrent ? 'text-t1' : isCompleted ? 'text-t3' : 'text-t3'
                    }`}
                  >
                    {b.title}
                  </p>
                  ...
                  <p className="text-[10px] text-t4">
                    ...
                    {!isUnlocked && prevBook && (
                      <span className="ml-1">
                        · Valide « {prevBook.title} » pour débloquer
                      </span>
                    )}
                  </p>
                </div>
